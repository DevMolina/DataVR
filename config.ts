...
export const CONFIG = {
  ...
  // --- Rango de letras iniciales de la placa ---
  // Cascada automáticamente desde el servidor activo (variable de entorno
  // SERVIDOR, ver src/config/servidores.ts) — cada Vía Rápida tiene su propio
  // rango asignado para no duplicar placas entre servidores. Para forzar un
  // rango distinto al de la tabla de servidores, sobreescribe el valor aquí
  // directamente (ej. { desde: 'A', hasta: 'Z' } o null para cualquier letra).
  RANGO_LETRA_INICIAL_PLACA: servidorActivo().rangoLetraInicialPlaca as { desde: string; hasta: string } | null,
